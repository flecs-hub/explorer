    ...
    template: `
      <span class="entity-reference">
        <span v-if="!disabled && !click_name">
          <template v-if="text && text.length">{{text}}&nbsp;</template><span>{{entity_name}}</span><template v-if="has_parent">&nbsp;-&nbsp;<entity-parent :entity="entity"/></template><icon src="follow" v-on:click.stop="clicked" v-if="show_icon"/>
        </span>
